...
本项目是一个 pnpm 管理的 TypeScript 组件库，基于 Ant Design v5 做企业级 UI 二次封装。目标是让业务项目从 `import * from 'antd'` 平滑切换到 `import * from 'pb-antd'`。
...
文档使用 dumi。全局指南放在 `docs/`，组件文档优先放在组件目录下的 `index.md`。组件文档应包含用途说明、基础示例、可配置示例和 API 说明。示例代码应使用 `pb-antd` 导入，体现业务项目真实用法。
